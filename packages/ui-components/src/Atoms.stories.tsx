...
const typographySamples = [
  { name: "Display XL", className: "momo-typo-display-xl" },
  { name: "Display LG", className: "momo-typo-display-lg" },
  { name: "Display MD", className: "momo-typo-display-md" },
  { name: "Display SM", className: "momo-typo-display-sm" },
  { name: "Heading XL", className: "momo-typo-heading-xl" },
  { name: "Heading LG", className: "momo-typo-heading-lg" },
  { name: "Heading MD", className: "momo-typo-heading-md" },
  { name: "Heading SM", className: "momo-typo-heading-sm" },
  { name: "Body LG", className: "momo-typo-body-lg" },
  { name: "Body MD", className: "momo-typo-body-md" },
  { name: "Body SM", className: "momo-typo-body-sm" },
  { name: "Label LG", className: "momo-typo-label-lg" },
  { name: "Label MD", className: "momo-typo-label-md" },
  { name: "Label SM Medium", className: "momo-typo-label-sm-medium" },
  { name: "Caption MD", className: "momo-typo-caption-md" },
];

const colorSamples = [
  { name: "MoMo Blue", value: "var(--color-momo-blue)", text: "var(--color-momo-yellow)" },
  { name: "MoMo Yellow", value: "var(--color-momo-yellow)", text: "var(--color-momo-blue)" },
  { name: "Brand Primary", value: "var(--color-brand-primary)", text: "var(--color-momo-yellow)" },
  { name: "Surface Default", value: "var(--color-surface-default)", text: "var(--color-text-default)" },
  { name: "Surface Secondary", value: "var(--color-surface-secondary)", text: "var(--color-text-default)" },
  { name: "Status Positive", value: "var(--color-status-positive)", text: "white" },
  { name: "Status Warning", value: "var(--color-status-warning)", text: "black" },
  { name: "Status Danger", value: "var(--color-status-danger)", text: "white" },
  { name: "Neo Gradient", value: "var(--color-neo-library-gradient-primary)", text: "white" },
];
...
const meta = {
  title: "Foundations/Atoms",
  tags: ["autodocs"],
  parameters: {
    layout: "fullscreen",
    backgrounds: {
      default: "foundation-canvas",
      values: [{ name: "foundation-canvas", value: "#f7f9fc" }],
    },
  },
} satisfies Meta;
      ...
      <div className="momo-content-width space-y-10">
        ...
        <section className="rounded-3xl border border-border-default bg-surface-default p-8 shadow-momo-sm md:p-10">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {colorSamples.map((sample) => (
              <div
                key={sample.name}
                className="rounded-xl border border-black/5 p-6 shadow-sm"
                style={{ background: sample.value, color: sample.text }}
              >
        ...
        <section className="rounded-3xl border border-border-default bg-surface-default p-8 shadow-momo-sm md:p-10">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {spacingSamples.map((token) => (
              <div key={token} className="rounded-xl border border-border-default bg-surface-secondary p-5">
                ...
                <div className="mt-3 h-1.5 rounded-full bg-momo-blue" style={{ width: `var(${token})`, minWidth: "2px" }} />
              </div>
            ))}
          </div>
        </section>
                ...
                <div className="mx-auto w-[320px] rounded-lg border border-border-default bg-surface-default p-[20px]">
                  <div className="momo-grid-mobile">
                    {Array.from({ length: 4 }).map((_, idx) => (
                      <div key={`m-${idx}`} className="h-16 rounded-md border border-momo-blue/20 bg-momo-yellow/45" />
                    ))}
                  </div>
                </div>
              </div>
                ...
                <div className="mx-auto w-[1120px] rounded-lg border border-border-default bg-surface-default p-4">
                  <div className="momo-grid-desktop">
                    {Array.from({ length: 12 }).map((_, idx) => (
                      <div key={`d-${idx}`} className="h-14 rounded-md border border-momo-blue/20 bg-momo-blue/20" />
                    ))}
                  </div>
                </div>
              </div>
